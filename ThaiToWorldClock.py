...
class Button:
    ...
    def reset():
        ...
        #Hide all time
    txtreset = "Reset"
    reset = Button(height=1, width=24, borderwidth=0, text=txtreset, fg="white", bg="#3C3C3C", command=reset)
    ...
    def close():
        Gui.master.quit()
    txtclos = "Close"
    close = Button(height=1, width=24, borderwidth=0, text=txtclos, fg="white", bg="#3C3C3C", command=close)

    #INITS : Display all Button
    reset.grid(row=6, column=0, columnspan=5, sticky=W, pady=0)
    execute.grid(row=6, column=2, columnspan=5, sticky=W, padx=13.5, pady=0)
